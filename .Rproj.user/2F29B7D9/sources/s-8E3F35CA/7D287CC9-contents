...
map_to_datatypes<-function(df,questionnaire){
  ...
  types[types_raw == "calculate"] <- "numerical"
  types[str_detect(types_raw$type,"validation_status|uuid|version_|diviceid|start|today|audit|note|begin|end|group")] <-NA
...
dap<-make_analysisplan_all_vars(data,questionnaire = questionnaire)

dap %>% write.csv("./input/dap.csv",row.names=F)
